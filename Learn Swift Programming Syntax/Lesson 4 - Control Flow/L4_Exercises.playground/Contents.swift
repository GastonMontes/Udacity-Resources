...
for rent in oldMountainViewRent {
    newMountainViewRent.append(1.2 * rent)
}
...
for (foodName, foodValue) in polarizingFoods {
    if foodValue {
        print("\(foodName), yum!")
    } else {
        print("\(foodName), yuck!")
...
for (areaName, phoneNumber) in rapperPhoneNumbers {
    rapperPhoneNumbers[areaName] = phoneNumber.replacingOccurrences(of: "415", with: "510")
}
