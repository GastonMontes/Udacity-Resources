...
let cuteAnimals = [CuddlyCreature]()
...
var admittedName = waitingList.remove(at: 0)
admitted.append(admittedName)

admittedName = waitingList.remove(at: 0)
admitted.append(admittedName)
...
let foodDict = ["Anchovies": true, "Coconut": true, "Cilantro": true, "Liver": false]
...
let georgeBushPetName = presidentialPetsDict.removeValue(forKey: "George Bush")
presidentialPetsDict["George W. Bush"] = georgeBushPetName
print(presidentialPetsDict)
...
if let obamaDog = presidentialPetsDict["Barack Obama"] {
    print("Michele Obama walks \(obamaDog) every morning.")
}
